...
export default function OrientationStrip({ onSelectSection }: OrientationStripProps) {
  ...
  return (
    <section
      ...
      className="w-full bg-[#161616] text-[#F3EFE9] py-12 sm:py-18 px-4 sm:px-8 lg:px-14 relative z-20 select-none border-b border-black/40"
    >
      <div className="max-w-7xl mx-auto flex flex-col gap-8 sm:gap-14">
        {/* Top Header Row */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <h2
            className="text-[clamp(28px,4.5vw,60px)] font-bold uppercase tracking-tight leading-[0.95] text-white reveal"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 sm:gap-6 lg:gap-8 pt-2">
          {steps.map((step, idx) => (
            <button
              key={step.num}
              onClick={() => handleScrollTo(step.targetId, idx)}
              className={`group text-left flex flex-col gap-1.5 sm:gap-2 p-2.5 sm:p-3 -m-2 sm:-m-3 rounded hover:bg-neutral-900/60 transition-colors cursor-pointer reveal ${
                idx === 4 ? "col-span-2 sm:col-span-1" : ""
              }`}
              style={{ "--delay": `${100 + idx * 80}ms` } as React.CSSProperties}
            >
              <span className="orientation-step-number text-[#FF5722] font-mono text-[12.5px] sm:text-[14px] font-semibold tracking-wider group-hover:translate-x-1 transition-transform inline-block">
                {step.num}
              </span>
              <span className="orientation-step-label text-white text-[14px] sm:text-[17px] font-semibold tracking-tight leading-snug">
                {step.label}
              </span>
            </button>
          ))}
